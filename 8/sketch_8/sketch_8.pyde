class Library(): 
    ...
    def lendBook(self, requestedBook): 
        if requestedBook in self.availableBooks: 
            ...
            print("Sorry, the book client want to borrow is not available in our list.")
...
def setup():
    size(220,100)
    global library, Madzia, Roksana
    books = ["Naocznosc", "Sens Sztuki", "Harry Potter", "Proces"]
...
def keyPressed(): #ciekawe rozróżnienie kto wypożycza ;)
        ...
        if key == CODED:     
                if keyCode == UP:
                    library.lendBook(Roksana.requestBook("Proces")) 
                if keyCode == DOWN:
                    library.addBook(Roksana.returnBook()) 
